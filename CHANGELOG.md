...
Licensing and attribution pass. No behavior change: the skill, its activation rules, and every reference path are untouched. What changed is the paperwork that a redistributor, a marketplace reviewer, or a vendor's legal team would actually read — and it changed because the previous version overstated what the project could grant and what it had verified.
...
- **Accuracy corrections found while verifying the sources**, which is the part of this pass that changes what the skill tells a user. Anthropic's general guide no longer carries the "up to 30%" long-context figure — the section now reads "This improves performance across all models" — so the number is gone from `framework.md`, `runtime-detection.md`, `universal-baseline.md` and `component-definitions.md`; a quantitative claim a vendor has withdrawn is the easiest kind of citation to attack. Three quotations in the `framework.md` mapping table were truncated or spliced without an ellipsis and now match the page. The xAI recommendation quote is on the models page, not the `grok-4-6` page, and says so. The OpenAI lean-prompt figures verified verbatim.
