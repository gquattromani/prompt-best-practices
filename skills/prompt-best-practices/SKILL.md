...
| `references/claude-opus-5-5.md` | The target is Claude Opus 5.5 — assume this when the user states no model — or Claude Opus 5. |
| `references/claude-fable-5.md` | The target is Claude Fable 5.1 / Mythos 5.1, or Fable 5 / Mythos 5. |
| `references/codex-considerations.md` | The target is OpenAI Codex or another OpenAI model (GPT-6 family, GPT-5.6 family). |
...
| `references/grok-considerations.md` | The target is xAI Grok (`grok-4.7`) — Grok Build, or a picker set to Grok. |
...
- **Claude** (Opus 5.5 default target; Fable 5.1 / Mythos 5.1 top tier; Opus 5 and Opus 4.8 previous generations) → XML tags (default, strongest). Start at `references/claude-considerations.md`, then load the per-model file.
- **Codex / OpenAI** (GPT-6 Sol default in Codex, GPT-6 Astra most capable) → Markdown headings or XML (both work); drop repeated instructions and non-behavioral examples; state the writing style and one authorization boundary that grants the autonomy the request implies. See `references/codex-considerations.md`.
...
- **xAI Grok** (`grok-4.7`) → keep Role and expand Constraints with edge cases (this vendor asks for a thorough prompt, not a lean one); enumerate context paths; describe tool use as native function calling, never as an XML envelope. See `references/grok-considerations.md`.
...
**Frontier-model calibration (Opus 5.5, Fable 5.1 / Mythos 5.1, GPT-6):** build the *smallest sufficient* prompt, not the most complete one. Their vendors explicitly reward leaner prompts and warn that over-prescription degrades output. A component earns its place only if it changes the output — examples are the first to cut, rules are stated once with their reason, and reasoning depth is set by the `effort` parameter rather than "think hard" text. Some legacy instructions are a *delete*, not a rewrite. See `references/framework.md` > "Calibrating for frontier models".
...
- **Claude Opus 5 / 5.5**: remove any self-check / verification / "double-check your answer" clause — including one the user's original prompt carried over — and note the removal in one line. Response length must be stated in `<output_spec>`; `effort` will not shorten it. On Opus 5.5 also remove "think carefully before answering" lines and any instruction to write the reasoning into the response.
- **Claude Fable 5.1 / Mythos 5.1** (and 5), long autonomous run: the opposite call — keep the verifier instruction, preferring a fresh-context verifier subagent over self-critique. On 5.1 also remove anti-formatting blocks and "hold all findings for the final response" lines: the model already formats less and writes fewer updates.
- **OpenAI GPT-6**: do not add a verification clause — the model tests thoroughly by default, so scope testing to the change; request delegation rather than capping it; state the writing style, because the default leans on lists and tables.
- **Google Gemini 3.x**: remove temperature and other sampling instructions (the vendor says to remove them from all requests, and lowering temperature can cause looping) and remove chain-of-thought scaffolding; add the explicit length and tone line. A "think very hard before answering" nudge is legitimate here on heavy-reasoning tasks only — it is the one runtime that endorses it.
...
- **Target runtime is Claude Opus 5.5 (the default when the model is unstated) or Opus 5**: consult `references/claude-opus-5-5.md` — prompt explicitly for conciseness (`effort` does not shorten visible output), delete verification and self-check clauses instead of rewording them, constrain scope on narrow tasks, cap subagent delegation; on Opus 5.5, thinking is always on, so thinking-disabled scaffolding and "think carefully" lines go, and unattended runs name the early stops to avoid. The 7 components still apply: Output specification and Constraints carry more weight, verification-style constraints are dropped.
...
- **Target runtime is xAI Grok (`grok-4.7`, the default model of Grok Build)**: consult `references/grok-considerations.md` — the one runtime that asks for a *thorough* prompt: keep Role, spell out edge cases in Constraints, enumerate the context paths, and never request tool calls in an XML envelope.
- **Target runtime is OpenAI Codex (GPT-6 or GPT-5.6 family)**: consult `references/codex-considerations.md` — outcome-first / leaner prompts (remove repeated instructions and non-behavioral examples), the `apply_patch` tool for edits, one authorization boundary that also grants autonomy on GPT-6, an explicit writing style, test scope instead of a verification step, `reasoning.effort` for depth. The 7 components still apply, but through the lean-prompt lens.
